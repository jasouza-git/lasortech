...
/** Actions */
const action = {
    'password_check': (pass:string) => {
        const req = [];
        console.log(pass);
        // Minimum eight characters, at least one letter and one number
        if (pass.length < 8) req.push('Minimum eight character');
        if (!/[0-9]/g.test(pass) || pass.match(/[0-9]/g).length < 1) req.push('At least one number');
        if (!/[a-zA-Z]/g.test(pass) || pass.match(/[a-zA-Z]/g).length < 1) req.push('At least one letter');
        if (req.length) throw pop('error', 'Invalid Password', `Please fulfill the following requirements:<ul>${req.map(x=>`<li>${x}</li>`)}</ul>`);
    },
    'signup': async () => {
        const [
            name,
